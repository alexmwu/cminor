...
debug:
	...
	$(CC) $(GDBFLAG) $(CCDEBUGFLAGS) -c $(GRAM_SRC) $(HANDLE_SRC) $(AST_SRC) $(CFLAGS)
	$(CC) $(GDBFLAG) $(CCDEBUGFLAGS) $(GRAM_OBJ) $(HANDLE_OBJ) $(AST_OBJ) -o $(OUTPUT_BINARY) $(CFLAGS)
...
$(SCANNER_SRC) $(SCANNER_PRE).yy.h: $(SCANNER_IN) $(TOKEN_HEADER) $(PARSER_PRE).tab.h
	$(LEX) $(LEXFLAGS) $(SCANNER_IN)
clean:
	rm $(SCANNER_PRE).yy.h $(PARSER_PRE).tab.h $(SCANNER_SRC) $(PARSER_SRC) $(PARSER_PRE).output $(GRAM_OBJ) $(HANDLE_OBJ) $(AST_OBJ) $(OUTPUT_BINARY)
